...
@WebServlet(name = "ch06_Join", urlPatterns = { "/servlet/member/join" })  // URL에는 왠만하면 대문자를 안쓰는 게 좋다
...
//		}
		...
		// 로그인을 건너뛰고 메인으로 갈 수있게 로그인 성공을 기록해주는 걸 추가해야함  => 쿠키 사용하기
		Cookie cookie = new Cookie("isLogin", "true");
		
		response.addCookie(cookie);
		
		// 회원가입 성공 여부 (회원가입 성공 URL을 치고 들어가면 회원가입을 안해도 회원가입 성공 페이지가 뜨는 것을 방지하기 위해)
		Cookie cookie2 = new Cookie("isJoin", "true");
		response.addCookie(cookie2);
		
		// 회원가입을 실제로 완료한 사람만 페이지로 이동
		// 이제는 회원가입 완료 html이 아닌 회원가입 완료 servlet이 된 것	
